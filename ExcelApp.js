...
(function(r) {
  var ExcelApp;
  ExcelApp = (function() {
    var a1NotationToRowCol, columnToLetter, disassembleExcel, getCommentsAsObject, getImagesAsObject, getNamedFunctionsAsObject, getQuotePrefixCellsAsObject, getSharedStrings, getStyleAsObject, getValuesAsObject, getValuesFromObj, getXmlObj, letterToColumn, parsXLSX, putError, putInternalError, sheetNameToFileName;
    ...
    getNamedFunctionsAsObject = function() {
      ...
      definedNames = root.getChild("definedNames", root.getNamespace()).getChildren();
    ...
    getQuotePrefixCellsAsObject = function() {
      ...
      if (this.mainObj.sheetsObj.sheetsObj.hasOwnProperty(this.obj.sheetName)) {
        ...
        if (this.mainObj.fileObj.hasOwnProperty(checkFilename)) {
          xmlObj1 = getXmlObj.call(this, "xl/styles.xml");
          styler = xmlObj1.getRootElement();
          quotePrefix = styler.getChild("cellXfs", styler.getNamespace()).getChildren().map((e) => {
            if (e.getAttribute("quotePrefix")) {
              return true;
            } else {
              return false;
            }
          });
          xmlObj2 = getXmlObj.call(this, checkFilename);
          sr = xmlObj2.getRootElement();
          return sr.getChild("sheetData", sr.getNamespace()).getChildren().reduce((ar, r, i) => {
            r.getChildren().forEach((c, j) => {
              var v;
              r = c.getAttribute("r").getValue();
              v = Number(c.getAttribute("s").getValue());
              if (quotePrefix[v]) {
                return ar.push(r);
              }
            });
    ...
    getSharedStrings = function() {
      ...
      if (this.mainObj.fileObj.hasOwnProperty(this.sharedStrings)) {
        ...
        this.mainObj.valueSharedStrings = rootChildren.map((e) => {
          var temp;
          temp = e.getChild("t", e.getNamespace());
          if (temp) {
    ...
    getCommentsAsObject = function() {
      var checkFilename, commentFilename, commentList, comments, f, i, p, ref1, root, rootChildren, t, target, temp, xmlObj;
      commentFilename = "";
      if (this.mainObj.sheetsObj.sheetsObj.hasOwnProperty(this.obj.sheetName)) {
        ...
        if (this.mainObj.fileObj.hasOwnProperty(checkFilename)) {
          xmlObj = getXmlObj.call(this, checkFilename);
          rootChildren = xmlObj.getRootElement().getChildren();
          for (i = p = 0, ref1 = rootChildren.length; (0 <= ref1 ? p < ref1 : p > ref1); i = 0 <= ref1 ? ++p : --p) {
    ...
    letterToColumn = function(letter) {
      var column, i, length, p, ref1;
      column = 0;
      length = letter.length;
      for (i = p = 0, ref1 = length; (0 <= ref1 ? p < ref1 : p > ref1); i = 0 <= ref1 ? ++p : --p) {
